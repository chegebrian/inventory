...
const ClerkDashboard = () => {
  ...
  const tableData = recentEntries.map((entry) => ({
    ...
    payment_status: (
      <span
        className={`px-3 py-1 text-xs rounded-full ${entry.payment_status === "paid" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
      >
      ...
      <div className="card">
        ...
        {recentEntries.length === 0 ? (
          <EmptyState
            ...
            message="Your recent records will appear here"
            icon="📋"
          />
        ) : (
